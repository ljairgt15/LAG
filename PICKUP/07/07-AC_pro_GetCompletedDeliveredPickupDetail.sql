    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SELECT 
                ...
                ,COUNT(1)
                ,CASE WHEN (UBB.IdBodega IS NULL OR UBB.IdBodega = '') THEN GH.IdBodega ELSE UBB.IdBodega END
                ,MD.Id
                ,PC.IdCarrier
                ,TRP.Nombre
                ,GH.IdGuia
            ...
            INNER JOIN GuiasHouseDetalles GHD WITH(NOLOCK) ON HST.IdGuiaHouseDetalle = GHD.Id 
            ...
            OUTER APPLY (
                SELECT TOP 1 DOC.EsPod, DOC.NombreArchivo, DOC.MailEnviado, DOC.PodProcesado
                FROM DocumentosDespacho DOC 
                WHERE DOC.IdManifiesto = MD.Id 
                  AND DOC.IdDocumento = 'DOC052395'
                ORDER BY DOC.EsPod DESC
            ) DCD
            OUTER APPLY (   
                SELECT TOP (1) SOV.Id, SOV.NroOrden
                FROM SolicitudDeVentaDetalles SVD
                LEFT JOIN SolicitudDeVenta SOV ON SOV.Id = SVD.IdSolicitud
                WHERE SVD.IdGuiaHouseDetalle = GHD.Id
                ORDER BY SOV.FechaSolicitud DESC
            ) SDV
            ...
            LEFT JOIN UbicacionesBodega UBB ON UBI.IdUbicacionBodega = UBB.Id
        ...
        BEGIN
            ...
            INNER JOIN Bodegas BOD ON FIN.IdBodega = BOD.Id
